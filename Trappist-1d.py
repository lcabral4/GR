...
def set_scene(r):     # r = init position of planet
    ...
    scene = vp.canvas(title='Precession of Trappist-1 d', 
...
def go(animate = False):                     # default: True
    r, v = np.array([0.0224927 , 0.0, 0.]), np.array([0.0, 12.491507083582626 , 0.]) # init r, v for merc
    ...
    plt.title('Precession Rate of Trappist-1 d Due to GR Over a Century')
    plt.plot(ta, angle)
    plt.plot(x,line, label='Precession Line')
    plt.legend()
    plt.xlabel('Time (year)'), plt.ylabel('Precession (arcsec)')
    plt.savefig('Trappist-1d.pdf')
    plt.show()
