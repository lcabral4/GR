...
def set_scene(r):     # r = init position of planet
    ...
    scene = vp.canvas(title='Precession of Trappist -1b', 
...
def go(animate = False):                     # default: True
    r, v = np.array([0.015958000000000003, 0.0, 0.]), np.array([0.0, 14.830179363169007, 0.]) # init r, v for merc
    ...
    while 0<=t<=10:  # run for 100 years
        rvec=vec(r[0],r[1],r[2])
    ...
    plt.title('Precession Rate of Trappist -1b Due to GR Over a Century')
    plt.plot(ta, angle)
    plt.plot(x,line, label='Precession Line')
    plt.legend()
    plt.xlabel('Time (year)'), plt.ylabel('Precession (arcsec)')
    plt.savefig('Trappist-1b.pdf')
    plt.show()
